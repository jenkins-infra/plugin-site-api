...
node('linux-arm64') {
    ...
    timestamps {
        stage('Generate Plugin Data') {
          infra.runMaven(['-PgeneratePluginData'], '25', null, true, !infra.isTrusted())
        }
        ...
        if (!infra.isInfra()) {
            ...
            */
                ...
                */
                stage('Build') {
                    withEnv([
                        ...
                        infra.runMaven(['-Dmaven.test.failure.ignore',  'verify'], '17', null, true, !infra.isTrusted())
                    }
            ...
            stage('Maven build') {
                infra.runMaven(['-Dmaven.test.skip=true',  'package'], '17')
                stash name: 'build', includes: 'target/*.war'
            }
            stage('Build and publish Docker image') {
                buildDockerAndPublishImage('plugin-site-api', [
                  unstash: 'build',
